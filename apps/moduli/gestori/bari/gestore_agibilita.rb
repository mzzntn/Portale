...
module Moduli
	...
	class GestoreAgibilita
	 ...
	 #        #File.open(File.join(Spider.paths[:tmp], 'moduli_dinamici','test.txt'), 'w+') { |file| file.write(hash_pratica.to_json) }
		...
		# 	#completare il caricamento dati
		...
		def self.carica_dati_modulo(params, hash_conf)
			...
			hsh_output['asseverazione'] = {}
			hsh_output['asseverazione'] = hsh_output['progettista_1'] unless hsh_output['progettista_1'].blank? 
			hsh_output
		end		
