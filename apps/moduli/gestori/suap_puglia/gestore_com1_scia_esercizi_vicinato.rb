...
module Moduli
	...
	class GestoreCom1SciaEserciziVicinato
	        ...
	        unless hash_pratica['dati_aggiuntivi'].blank?
				...
				unless settmerc.blank?
					hsh_output["sezione_c"].merge!({ "nuova_sup_vendita" => {settmerc => {'check' => '1'} },
												"nuova_sup_vendita_c2" => {settmerc => {'check' => '1'} }
												 })
				end
				special = hash_pratica['dati_aggiuntivi']['SPECIAL']
				unless special.blank?
					hsh_output["sezione_c"].merge!({ "tab_speciali" => {special => {'check' => '1'} }
												 })
				end
				...
				settmerc = hash_pratica['dati_aggiuntivi']['SETTMERC']
				unless settmerc.blank?
					hsh_output["sezione_comune_d"]["SETTMERC"] = { settmerc => {} } 
					hsh_output["sezione_comune_d"]["SETTMERC"][settmerc]['check'] = '1'
				end
				caratt = hash_pratica['dati_aggiuntivi']['CARATT']
				unless caratt.blank?
					hsh_output["sezione_comune_d"]["CARATT"] = { caratt => {} } 
					hsh_output["sezione_comune_d"]["CARATT"][caratt]['check'] = '1'
				end
				special = hash_pratica['dati_aggiuntivi']['SPECIAL']
				unless special.blank?
					hsh_output["sezione_comune_d"]["SPECIAL"] = { special => {} } 
					hsh_output["sezione_comune_d"]["SPECIAL"][special]['check'] = '1'
				end
		...
		def self.set_scene_widget(scene, hash_dati=nil)
			scene.id_modulo_com = '1'
			scene.nome_modulo = 'Esercizi di Commercio al dettaglio di vicinato'
