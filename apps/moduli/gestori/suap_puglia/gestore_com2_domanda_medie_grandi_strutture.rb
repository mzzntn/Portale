...
module Moduli
	...
	class GestoreCom2DomandaMedieGrandiStrutture
		...
		def self.carica_dati_modulo(params, hash_conf)
	        ...
	        unless hash_pratica['dati_aggiuntivi'].blank?
				...
				unless settmerc.blank?
					hsh_output["sezione_b"].merge!({ "nuova_sup_vendita" => {settmerc => {'check' => '1'} },
												"nuova_sup_vendita_b2" => {settmerc => {'check' => '1'} },
												"accorp_sup_vendita_b3" => {settmerc => {'check' => '1'} },
												"sup_prevista" => {settmerc => {'check' => '1'} },
												"elim_settore_b4" => {settmerc => {'check' => '1'} }
												 })
				end
				special = hash_pratica['dati_aggiuntivi']['SPECIAL']
				unless special.blank?
					hsh_output["sezione_b"].merge!({ "tab_speciali_b2" => {special => {'check' => '1'} },
												"tab_speciali_b3" => {special => {'check' => '1'} },
												"tab_speciali_b4_aggiunto" => {special => {'check' => '1'} },
												"tab_speciali_b4_prevista" => {special => {'check' => '1'} }
												 })
				end
			end
		...
		def self.set_scene_widget(scene, hash_dati=nil)
			scene.id_modulo_com = '2'
			scene.nome_modulo = 'Esercizi di Commercio al dettaglio - Medie e Grandi strutture di vendita.'
