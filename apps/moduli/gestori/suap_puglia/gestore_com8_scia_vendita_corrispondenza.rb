...
module Moduli
	...
	class GestoreCom8SciaVenditaCorrispondenza
		...
		def self.carica_dati_modulo(params, hash_conf)
	        ...
	        unless hash_pratica['dati_aggiuntivi'].blank?
				...
				unless settmerc.blank?
					hsh_output["sezione_a"].merge!({ "settore_merce" => {settmerc => {'check' => '1'} }
												 })
					hsh_output["sezione_b"].merge!({ "settore_merce" => {settmerc => {'check' => '1'} }
												 })
					hsh_output["sezione_c"].merge!({ "settore_merce" => {settmerc => {'check' => '1'} },
													 "settore_merce_c1" => {settmerc => {'check' => '1'} }
												 })
					hsh_output["sezione_d"].merge!({ "settore_merce" => {settmerc => {'check' => '1'} }
												 })
		...
		def self.set_scene_widget(scene, hash_dati=nil)
			scene.id_modulo_com = '8'
			scene.nome_modulo = 'Forme speciali di vendita al dettaglio <br/ > VENDITA PER CORRISPONDENZA, TELEVISIONE O ALTRI SISTEMI DI COMUNICAZIONE '
