...
module Moduli
	...
	class GestoreCom9SciaCommercioElettronico
		...
		def self.carica_dati_modulo(params, hash_conf)
	        ...
	        unless hash_pratica['dati_aggiuntivi'].blank?
				...
				unless settmerc.blank?
					...
					hsh_output["sezione_c"].merge!({ "settore_merce_trasferito" => {settmerc => {'check' => '1'} }
												 })
		...
		def self.set_scene_widget(scene, hash_dati=nil)
			scene.id_modulo_com = '9'
			scene.nome_modulo = 'Forme speciali di vendita al dettaglio - COMMERCIO ELETTRONICO'
