...
?>
  <form class="form-ricerca form-horizontal col-lg-12 col-md-12 col-sm-12 col-xs-12" action='<?=$_SERVER['PHP_SELF']?>' method='<?=$W->config['method']?>' id='<?=$D->name?>_form'>
    ...
    <input type="hidden" name="<?=str_replace("search","tabella",$D->name)?>[start]" value="0">
	  ...
	  <div class="row">
	      <label for="<?=$i->DATA_AFFIS_1->htmlName?>" class="sr-only">Data pubblicazione</label>
	      <label class="col-lg-2 col-md-2 col-sm-2 control-label-left">Data pubblicazione</label>
	      <div class="col-lg-4 col-md-4 col-sm-4">
		  <? $W->inputs->DATA_AFFIS_1->display(); ?>
	      </div>

	      <label for="<?=$i->DATA_AFFIS_2->htmlName?>" class="sr-only">fino a</label>
	      <label class="col-lg-2 col-md-2 col-sm-2 control-label-left">fino a</label>
	      <div class="col-lg-4 col-md-4 col-sm-4">
		  <? $W->inputs->DATA_AFFIS_2->display(); ?>
	      </div>
	  </div>
      </div>
      <div class="form-group">
	  <div class="row">
	      ...
	      <label for="<?=$i->TIPOATTO->htmlName?>" class="sr-only">Tipo atto</label>
	      <label class="col-lg-2 col-md-2 col-sm-2 control-label-left">Tipo atto</label>
	      <div class="col-lg-4 col-md-4 col-sm-4">
...
<form action='<?=$_SERVER['PHP_SELF']?>' method='<?=$W->config['method']?>' id='<?=$D->name?>_form' >
    <p class='multi'>  
	    <label for='<?=$i->DATA_AFFIS_1->htmlName?>'>Data pubblicazione</label><? $W->inputs->DATA_AFFIS_1->display(); ?>
        <label for='<?=$i->DATA_AFFIS_2->htmlName?>' class='inline'>fino a</label> <? $W->inputs->DATA_AFFIS_2->display(); ?>
	</p>
	<p> 
