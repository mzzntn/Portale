...
?>
  <form class="form-ricerca form-horizontal<?=$W->config['hidable']?" form-hidable":""?> col-lg-12 col-md-12 col-sm-12 col-xs-12" action='<?=$_SERVER['PHP_SELF']?>' method='<?=$W->config['method']?>' id='<?=$D->name?>_form'>
    ...
    <input type="hidden" name="<?=str_replace("search","table",$D->name)?>[start]" value="0">
      ...
      <div class="form-group">
	  <div class="row">
	      <label for="<?=$i->indirizzi->inputs->via->htmlName?>" class="sr-only">Indirizzo (senza civico)</label>
	      <label class="col-lg-2 col-md-2 col-sm-2 control-label-left">Indirizzo (senza civico)</label>
	      <div class="col-lg-4 col-md-4 col-sm-4">
...
<form action='<?=$_SERVER['PHP_SELF']?>' method='<?=$W->config['method']?>' id='<?=$D->name?>_form' >
   <p class='multi'>
    ...
    <? $i->tipo->display() ?>
    <label for='<?=$i->tipo->htmlName?>' class='inline'>Numero:</label>
    <? $i->cod2->display('size=3') ?>
    <label for='<?=$i->tipo->htmlName?>' class='inline'>Anno:</label>
    <? $i->cod3->display() ?>
  </p>

  <p class='multi'>  
    <label for='<?=$i->inizio->htmlName?>'>Data</label>
    <label class='inline'>da:</label><? $W->inputs->inizio_1->display(); ?><label class='inline'>a:</label> <? $W->inputs->inizio_2->display(); ?>
  </p>
  <p>
  ...
  <p>
	...
	<p>
    <label for='<?=$i->indirizzi->inputs->via->htmlName?>'>Indirizzo (senza civico)</label>
	  <? $i->indirizzi->inputs->via->display() ?>
  </p>
  <p class='multi'>
