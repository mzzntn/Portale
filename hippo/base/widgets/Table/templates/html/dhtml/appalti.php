...
//        if ( strlen($descrizione) > 100) $descrizione = substr($descrizione, 0, 100).'...';
	$loader = & $IMP->getLoader('benefici::beneficio');
	$loader->addParam('id', $W->data->get('id'));
	$loader->requestAll();
	$beneficio = $loader->load();
    ?>
	<tr class="paginated_element">
	  <td><a href='<?= $W->config['admin'] ?><?=$W->data->get('id')?>'>Vai</a></td>
	  <td align='center'><?=$W->data->get('cig')?></td>
	  <td align='center'><?=$descrizione?></td>
	  <td align='center'><?=$beneficio->get('sceltaContraente')?></td>
