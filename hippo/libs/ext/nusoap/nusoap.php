...
class nusoap_parser extends nusoap_base {
	...
	*/
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		$this->appendDebug($this->varDump($xml));
		
		// rimpiazzo i caratteri speciali all'interno delle tag html, prima di usare xml_parser
		/*preg_match_all('/(">)([^<\n]+)(<)/u', $xml, $values);
		$results = count($values[0]);
		for($i = 0; $i<$results; $i++) {
		  $xml = str_replace($values[0][$i], "{$values[1][$i]}".StringParser::parse($values[2][$i],true)."{$values[3][$i]}", $xml);
		}
		$this->debug("xml escaped is:");
		$this->appendDebug($this->varDump($xml));
		$this->xml = $xml;*/
...
// 				$this->appendDebug($this->varDump($xml));
	...
	*/
	function character_data($parser, $data){
// 		$this->debug("inside character_data()");
		$pos = $this->depth_array[$this->depth];
		if ($this->xml_encoding=='UTF-8'){
			...
			/*if($this->decode_utf8){
				$this->debug("decoding utf for \$data $data");
				$data = utf8_decode($data);
				$this->debug("\$data now is $data");
			}*/
// 			$this->debug("parsing \$data $data");
			$data = StringParser::parse($data,true);
// 			$this->debug("\$data now is $data");
...
// 				$this->debug('&$this->message[$child_pos][\'result\'] '.print_r($this->message[$child_pos]['result'],true));
                	}
                }
			}
