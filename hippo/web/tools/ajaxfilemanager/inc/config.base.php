<?php
#include_once(dirname(__FILE__).'/../../../../../data/files');
...
//error_reporting(E_ALL ^ E_NOTICE);	
    ...
    // define('CONFIG_SYS_DEFAULT_PATH', PATH_WEBDATA); //accept relative path only
    // define('CONFIG_SYS_ROOT_PATH', PATH_WEBDATA);	//accept relative path only
    if (!session_id()) session_start();
	$fileType = $_GET['fileType'];
	if ($fileType) $_SESSION['fileType'] = $fileType;
    else $fileType = $_SESSION['fileType'];
    if ($fileType == 'image'){
        define('CONFIG_SYS_DEFAULT_PATH', PATH_WEBDATA.'/img/orig');
    	define('CONFIG_SYS_ROOT_PATH', PATH_WEBDATA.'/img/orig');
    }
    elseif($fileType == 'media'){
        define('CONFIG_SYS_DEFAULT_PATH', PATH_WEBDATA.'/media');
    	define('CONFIG_SYS_ROOT_PATH', PATH_WEBDATA.'/media');
    }
    else{
       define('CONFIG_SYS_DEFAULT_PATH', PATH_WEBDATA.'/files');
    	define('CONFIG_SYS_ROOT_PATH', PATH_WEBDATA.'/files');
    }    
	define('CONFIG_SYS_FOLDER_SHOWN_ON_TOP', true); //show your folders on the top of list if true or order by name 
	define("CONFIG_SYS_DIR_SESSION_PATH", VARPATH.'/session/');
	...
	define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,bmp,tif,zip,php,sit,rar,gz,tar,htm,html,mov,mpg,avi,asf,mpeg,wmv,aif,aiff,wav,mp3,swf,ppt,rtf,doc,pdf,xls,txt,xml,xsl,dtd,odt,ods,odp,xlsx,docx,pptx,csv');//
