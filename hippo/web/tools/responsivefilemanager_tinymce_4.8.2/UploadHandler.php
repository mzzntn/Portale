...
class UploadHandler
{
	...
	public function __construct($options = null, $initialize = true, $error_messages = null) {
        #file_put_contents("/var/svn/hippo/php5/web/tools/responsivefilemanager_tinymce_4.8.2/debug.log","uploadhandler: ".print_r($options,true),FILE_APPEND);
		$this->response = array();
		$this->options = array(
			...
			if ($uploaded_file && is_uploaded_file($uploaded_file)) {
				...
				if ($append_file) {
					if(file_put_contents(
						...
						// error
						$file->error = $this->get_error_message(7);
					}
				} else {
					if(move_uploaded_file($uploaded_file, $file_path)){} else{
						$file->error = $this->get_error_message(7);
					}
				}
			} else {
				if($this->get_file_size($uploaded_file) == $file->size){
					if(copy($uploaded_file, $file_path)){}else{$file->error = $this->get_error_message(7);}
				}else{
					...
					)){}else{$file->error = $this->get_error_message(7);}
				}
			}
