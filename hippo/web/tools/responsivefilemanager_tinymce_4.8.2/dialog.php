...
$thumbs_url 	= $config['thumbs_base_url'].$rfm_subfolder;
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			foreach ($files as $nu=>$file_array) {
				...
				if(!$ftp){
					$file_path=$config['current_path'].$rfm_subfolder.$subdir.$file;
					$file_url=$config['current_url'].$rfm_subfolder.$subdir.$file;
				...
				if(in_array($file_array['extension'], $config['ext_img'])){
					...
					if($ftp){
						...
						$src_thumb = $mini_src = $thumbs_url.$subdir.$file;
						$creation_thumb_path = $thumbs_path.$subdir. $file;
// 						echo "src_thumb is $src_thumb<br>";

						if(!file_exists($creation_thumb_path) ){
//                             echo "trying to create thumbnail in ".$creation_thumb_path."<br>";
							if(!create_img($file_path, $creation_thumb_path, 122, 91,'crop',$config)){
								$src_thumb=$mini_src="";
// 								echo "src_thumb changed to $src_thumb<br>";
							}
						}
