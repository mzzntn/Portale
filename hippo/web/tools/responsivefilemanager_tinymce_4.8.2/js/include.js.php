...
(function ($, Modernizr, image_editor)
{
	...
	var FileManager = {
		...
		contextActions: {
			...
			unzip: function($trigger)
			{
				...
				$.ajax({
					type: "POST",
					url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=extract",
					data: {
						path: m
					}
				}).done(function (msg)
			...
			jQuery('#filter-input').on('keyup', function ()
			{
				...
				if (js_script)
				{
					delay(function ()
					{
						...
						$.ajax({
							url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=filter&type=" + val
						}).done(function (msg)
						{
							if (msg != "")
							{
		...
		uploadURL: function()
		{
			jQuery('#uploadURL').on('click',function(e){
				...
				}).done(function (msg)
				{
					...
					hide_animation();
					jQuery('#url').val('');
					if((message.error && message.error!="") || (message.files && message.files[0].error && message.files[0].error!="")) {
						var details = "";
						if(message.files && message.files[0].error && message.files[0].error=="Filetype not allowed") {
							details = ": formato del file non consentito";
						}
						$("<p class='alert alert-danger'>si &egrave; verificato un errore durante il caricamento del file"+details+"</p>").insertAfter($('#uploadURL'));
					} else {
						$("<p class='alert alert-success'>file caricato con successo, per vederlo torna alla lista dei file</p>").insertAfter($('#uploadURL'));
					}
				}).fail(function(msg){
					$("<p class='alert alert-danger'>"+jQuery('#lang_error_upload').val()+"</p>").insertAfter($('#uploadURL'));
					hide_animation();
					jQuery('#url').val('');
				});
			})
		...
		makeSort: function(js_script)
		{
			...
			jQuery('.sorter').on('click', function ()
			{
				...
				{
					$.ajax({
						url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=sort&sort_by=" + _this.attr('data-sort') + "&descending=" + (sortDescending ? 1 : 0)
					});
	...
	jQuery(document).ready(function ()
	{
		...
		jQuery('.new-folder').on('click', function ()
		{
			bootbox.prompt(jQuery('#insert_folder_name').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (name)
			{
				if (name !== null)
				{
					...
					$.ajax({
						type: "POST",
						url: "execute.php?c=<?=$_GET["c"]?>&action=create_folder",
						data: {
							path: folder_path,
							name: name
						}
		...
		jQuery('.view-controller button').on('click', function ()
		{
			...
			$.ajax({
				url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=view&type=" + _this.attr('data-value')
			}).done(function (msg)
			{
				if (msg != "")
				{
	...
	function create_text_file()
	{
		...
		$.ajax({
			type: "GET",
			url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=new_file_form"
				...
				},
				{
					...
					"callback": function ()
					{
						...
						if (newFileName !== null)
						{
							...
							$.ajax({
								type: "POST",
								url: "execute.php?c=<?=$_GET["c"]?>&action=create_file",
								data: {
									path: folder_path,
									name: newFileName,
	...
	function edit_text_file($trigger)
	{
		...
		$.ajax({
			type: "POST",
			url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=get_file&sub_action=edit&preview_mode=text",
			data: { path: full_path }
				...
				[
					...
					},
					{
						...
						"callback": function ()
						{
							...
							$.ajax({
								type: "POST",
								url: "execute.php?c=<?=$_GET["c"]?>&action=save_text_file",
								data: {
									path: full_path,
									new_content: newContent
								}
	...
	function change_lang()
	{
		$.ajax({
			type: "POST",
			url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=get_lang",
			data: {}
				...
				[
					...
					},
					{
						...
						"callback": function ()
						{
							...
							$.ajax({
								type: "POST",
								url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=change_lang",
	...
	function chmod($trigger)
	{
		...
		$.ajax({
			type: "POST",
			url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=chmod",
			data: {
				path: full_path,
					...
					},
					{
						...
						"callback": function ()
						{
							...
							if (newPerm != '' && typeof newPerm !== "undefined")
							{
								...
								$.ajax({
									type: "POST",
									url: "execute.php?c=<?=$_GET["c"]?>&action=chmod",
									data: {
										path: full_path,
										new_mode: newPerm,
										is_recursive: recOpt,
	...
	function clear_clipboard()
	{
		bootbox.confirm(jQuery('#lang_clear_clipboard_confirm').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (result)
		{
			if (result == true)
			{
				$.ajax({
					type: "POST",
					url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=clear_clipboard",
					data: {}
				}).done(function (msg)
				{
					if (msg != "")
	...
	function copy_cut_clicked($trigger, atype)
	{
		...
		$.ajax({
			type: "POST",
			url: "ajax_calls.php?c=<?=$_GET["c"]?>&action=copy_cut",
			data: {
				path: full_path,
				sub_action: atype
			}
	...
	function paste_to_this_dir(dnd)
	{
		bootbox.confirm(jQuery('#lang_paste_confirm').val(), jQuery('#cancel').val(), jQuery('#ok').val(), function (result)
		{
			if (result == true)
			{
				...
				$.ajax({
					type: "POST",
					url: "execute.php?c=<?=$_GET["c"]?>&action=paste_clipboard",
					data: {
						path: folder_path
					}
				}).done(function (msg)
...
// because of feedback and on error bahhhhh...
	...
	apply = function(files, external)
	{
		...
		else
		{
			for(var i = 0; i< urls.length; i++){
                ...
                var selected_text = "<?=base64_decode($_GET['text'])?>";
				...
				else
				{
					...
					else
					{
						...
						{
                            if(selected_text!=""){alt_name = selected_text;}
	...
	function execute_action(action, file, name, container, function_name)
	{
		if (name !== null)
		{
			...
			$.ajax({
				type: "POST",
				url: "execute.php?c=<?=$_GET["c"]?>&action=" + action,
				data: {
					path: file,
					name: name.replace('/', '')
				}
